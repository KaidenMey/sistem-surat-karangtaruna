...
@section('title', 'Edit Template - E-SKATA')
...
@section('content')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $template->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="file" class="block text-sm font-medium text-gray-700 mb-2">File Template Baru (Kosongkan jika tidak diubah)</label>
                    <input type="file" id="file" name="file" accept=".docx"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @if($template->file_path)
                        <p class="mt-1 text-sm text-gray-600">File saat ini: {{ basename($template->file_path) }}</p>
                    @endif
                    <p class="mt-1 text-sm text-gray-500">Format: DOCX (Max: 10MB)</p>
                    @error('file')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <input type="hidden" name="form_fields" id="formFieldsJson" value="{{ old('form_fields', json_encode($template->form_fields ?? [])) }}">
                </div>
    ...
    <script>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select name="field_type_${fieldIndex}" class="w-full border border-gray-300 rounded px-3 py-2 text-sm" required>
                                ${Object.entries(fieldTypes).map(([value, label]) => 
                                    `<option value="${value}" ${fieldData?.type === value ? 'selected' : ''}>${label}</option>`
                                ).join('')}
                            </select>
                        </div>
                        <div class="flex items-end">
                            <div class="flex items-center space-x-2 w-full">
                                <label class="flex items-center">
                                    <input type="checkbox" name="field_required_${fieldIndex}" ${fieldData?.required ? 'checked' : ''}
        ...
        const existingFields = {!! json_encode($template->form_fields ?? []) !!};
